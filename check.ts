...
function printResult(result: StepResult): void {
  const icon = result.success
    ...
    : "\x1b[31m✗\x1b[0m";
  console.log(
    `${icon} ${result.name} (${formatDuration(result.duration)})`
  );
...
async function typecheckAll(
  ...
  const processes = typecheckProjects.map(
    async (project) => {
      ...
      const duration =
        (performance.now() - projectStart) / 1000;
  ...
  if (verbose) {
    for (const r of results) {
      ...
      const name = r.project
        ...
        .replace("/", "-");
      console.log(
        `  ${icon} ${name} (${formatDuration(r.duration)})`
      );
    }
  }
 ...
 */
async function effectDiagnostics(
  verbose: boolean
): Promise<{ success: boolean; output?: string }> {
...
async function runSingle(
  ...
  switch (command) {
    case "lint":
      ...
      break;
    case "typecheck":
      result = await runStep("typecheck", () =>
        typecheckAll(verbose)
      );
      break;
    case "format":
    ...
    case "test":
      ...
      break;
    case "effect":
      result = await runStep("effect", () =>
        effectDiagnostics(verbose)
      );
      if (result.success && result.output) {
    ...
    default:
      ...
      console.log(
        "Usage: bun check.ts [command] [--verbose]"
      );
      console.log(
        "Commands: all, lint, typecheck, effect, format, test, ci"
      );
      process.exit(1);
  }
