 ...
 */
async function getTsconfigHash(
  cwd: string,
  projects: string[]
): Promise<string> {
  const tsconfigPaths = [
 ...
 */
async function findEffectFiles(
  project: string
): Promise<EffectFile[]> {
...
function convertDiagnostic(
  ...
  const { line, character } =
    file.getLineAndCharacterOfPosition(diag.start ?? 0);
...
export async function runEffectDiagnostics(
  ...
  const fileResults = await Promise.all(
    projects.map((project) => findEffectFiles(project))
  );

  for (const files of fileResults) {
    for (const file of files) {
      allEffectFiles.push(file);
      projectFileCounts.set(
        file.project,
        (projectFileCounts.get(file.project) ?? 0) + 1
      );
    }
  }
  ...
  const lockfileHash = (await lockfile.exists())
    ...
    : "no-lockfile";
  const currentHash = computeCacheHash(
    allEffectFiles,
    tsconfigHash,
    lockfileHash
  );
  ...
  const baseTsconfig = ts.readConfigFile(
    `${cwd}/tsconfig.base.json`,
  ...
  const compilerOptions: ts.CompilerOptions = {
    ...
    paths: {
      ...
      "@map-poster/*": [`${cwd}/packages/*/src`],
    },
  };

  // Step 3: Create file map for LanguageServiceHost
  const fileContents = new Map<string, string>();
  const fileNames = allEffectFiles.map(
    (f) => f.absolutePath
  );
  ...
  for (const file of allEffectFiles) {
    ...
    projectDurations.set(
      file.project,
      (projectDurations.get(file.project) ?? 0) +
        fileDuration
    );
    ...
    for (const diag of semanticDiags) {
      ...
      if (converted) {
        const projectDiags =
          diagnosticsByProject.get(file.project) ?? [];
        projectDiags.push(converted);
        diagnosticsByProject.set(
          file.project,
          projectDiags
        );
      }
  ...
  // Step 8: Save cache only if diagnostics passed (no errors/warnings/messages)
  const hasIssues =
    totalErrors > 0 ||
    totalWarnings > 0 ||
    totalMessages > 0;
