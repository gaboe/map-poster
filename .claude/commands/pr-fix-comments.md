...
```bash
gh api repos/{owner}/{repo}/pulls/<pr_number>/comments | jq -r '[.[] | {id, in_reply_to_id, author: .user.login, body: (.body | .[0:60])}] | group_by(.in_reply_to_id // .id) | .[] | "Thread \(.[0].in_reply_to_id // .[0].id): \(length) comments"'
```
