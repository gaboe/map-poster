---
description: Systematic pre-PR code review for vivus codebase covering TRPC, TanStack, security, and performance
---
...
Systematic code review for pull request readiness in the vivus codebase. Identifies critical issues with focus on TRPC patterns, TanStack Router, Drizzle ORM, and security best practices.
...
- Ensure code follows vivus conventions from CLAUDE.md
...
**Common duplication locations to check:**

```typescript
// ❌ Wrong - Same logic in diagnostics.ts and k8s-metrics.ts
// diagnostics.ts
let maxRestarts = 0;
...
// k8s-metrics.ts  
const sorted = [...statuses].sort((a, b) => b.restartCount - a.restartCount);
const name = sorted[0]?.name;

// ✅ Correct - Shared utility in parsers.ts or utils.ts
export function findRestartingContainerName(
  statuses: Array<{ name: string; restartCount: number }>,
  specContainers: Array<{ name?: string }>
): string { ... }
```
...
- `packages/services/src/k8s/parsers.ts` - K8s parsing utilities
...
- **Imports:** Always absolute (`@/path` or `@vivus/*`), never relative in `apps/web-app/src`
...
- **Logger:** Use `@vivus/logger` for backend, `console.log/error` is OK for frontend React components
...
- `console.error` in React components (frontend) - Only backend code requires `@vivus/logger`

```typescript
// ✅ Correct
import { logger } from "@vivus/logger";
